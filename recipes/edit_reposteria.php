...
<head>
    ...
    <title>Editar Receta - Le Concassé</title>
...
<body>
     ...
     <nav class="navbar navbar-expand-sm" id="mainNav">
        ...
        <main>
        <h2 class="text-center title" style= "margin-bottom: -10px;"> Editar Receta Repostería</h2>
<div class="container mt-5 text-center">
        <?php foreach($query_rep as $q){ ?>
            <form enctype="multipart/form-data" method="POST">
            <label for="file">Imagen de la receta</label>
            <input type="file" required="required" accept="image/*" id= "file_rep" name="file_rep">
                <input type="text" hidden value='<?php echo $q['id']?>' name="id">
                <input type="text" placeholder="Titulo de la receta" class="form-control my-3 bg-dark text-white text-center" name="title_rep" value="<?php echo $q['recipe_name']?>">
                <textarea name="content_rep" placeholder="Receta" class="form-control my-3 bg-dark text-white" cols="30" rows="10"><?php echo $q['recipe_descrip']?></textarea>
                <textarea name="ingredient_rep" placeholder="Ingredientes" class="form-control my-3 bg-dark text-white" cols="30" rows="10"><?php echo $q['recipe_ingredients']?></textarea>
                <button class="btn btn-success" name="update_rep">Guardar Cambios</button>
            </form>
            <br>
