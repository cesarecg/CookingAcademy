...
<head>
    ...
    <title>Editar Noticia - Le Concassé</title>
...
<body>
     ...
     <nav class="navbar navbar-expand-sm" id="mainNav">
        ...
        <main>
        <h2 class="text-center title" style= "margin-bottom: -10px;"> Editar Noticia</h2>
<div class="container mt-5 text-center">
        <?php foreach($query_news as $q){ ?>
            <form enctype="multipart/form-data" method="POST">
            <label for="file">Imagen de la noticia</label>
            <input type="file" required="required" accept="image/*" id= "file_new" name="file_new">
                <input type="text" hidden value='<?php echo $q['id']?>' name="id">
                <input type="text" placeholder="Titulo de la noticia" class="form-control my-3 bg-dark text-white text-center" name="title_new" value="<?php echo $q['news_name']?>">
                <textarea name="content_new" class="form-control my-3 bg-dark text-white" cols="30" rows="10"><?php echo $q['news_descrip']?></textarea>
                <button class="btn btn-success" name="update_news">Guardar Cambios</button>
            </form>
            <br>
