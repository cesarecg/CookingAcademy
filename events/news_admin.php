...
<br>
  ...
  <?php if(isset($_REQUEST['info'])){ ?>
        ...
        <div class="text-center">
            <a href="create_news.php" class="btn btn-success">+ Crear una noticia nueva</a>
        </div>
            ...
            <?php foreach($query_news as $q){ ?>
                <div class="col-12 col-lg-4 d-flex justify-content-center">
                    <div class="card text-white bg-dark mt-5" style="width: 18rem;">
                        <div class="card-body">
                            <h5 class="card-title"><?php echo $q['news_name'];?></h5>
                            <p class="card-text"><?php echo substr($q['news_descrip'], 0, 50);?>...</p>
                            <div class="d-flex mt-2 justify-content-center align-items-center">
                            <a href="edit_news.php?id=<?php echo $q['id']?>" class="btn btn-light btn-sm" name="edit">Editar</a>
                            <form method="POST">
                            <input type="text" hidden value='<?php echo $q['id']?>' name="id">
                            <input type="text" hidden value='<?php echo $q['img_url']?>' name="img_url">
                            <button class="btn btn-danger btn-sm ml-2" onclick="return confirm('¿Estás seguro de borrar esta noticia? No habrá manera de recuperarlo');" name="delete_news">Borrar</button>
                            </form>
                            </div>
                        </div>
                    </div>
